...
def _parse_provinces_file(map_name: str) -> list[tuple[int, tuple[int, int, int], str | None]]:
    ...
    rows: list[tuple[int, tuple[int, int, int], str | None]] = []
    ...
    with open(province_file_path, "r", encoding="utf-8") as file:
        for line_num, line in enumerate(file, start=1):
            ...
            try:
                ...
                rows.append((province_id, rgb, terrain))
...
def load_provinces(map_name: str) -> dict[tuple[int, int, int], int]:
    ...
    for province_id, rgb, _terrain in _parse_provinces_file(map_name):
...
def load_province_terrains(map_name: str) -> dict[int, str]:
    ...
    for province_id, _rgb, terrain in _parse_provinces_file(map_name):
