...
export default function ResearchPage() {
  return <WikiPage title="Research" width="lg" intro="Research is an item-based deduction puzzle. Make a research paper at a Cartography Table, study it at a Lectern, and test materials to uncover and fill its hidden aspects.">
    ...
    <ol className="mt-4 list-decimal space-y-3 pl-6 text-sm text-[var(--tfmc-mist)]">
      <li>Sneak-right-click a Cartography Table to open the Research Station. Combine one vanilla Paper and one Alchemy Powder for one Parchment Paper. It takes 5 seconds.</li>
      <li>Combine one Parchment Paper with the subject ingredient. Ordinary papers take 5 seconds; an Unknown Research Paper takes 15 seconds. Every station recipe grants 10 Herborist EXP. Recipes hide when you lack their ingredients.</li>
      <li>Hold the resulting paper and right-click a Lectern. Each lectern has one owner, so find another lectern if one is occupied. A Staff Runestone can start its own project without first becoming a paper.</li>
    ...
    <Callout title="A concrete experiment example">Arcane Crystal is a primary Arcane item, so it supplies 2 Arcane points when tested. Crying Obsidian is secondary Arcane and supplies 1. These are distinct items, so each can be tested once. The Arcane Crystal project requires 3 Arcane points, plus its Earth, Energy and Machine requirements.</Callout>
    ...
    <SeeAlso hrefs={["/wiki/magic","/wiki/codex","/wiki/gem-infusion","/wiki/materials" /* TEMPORARILY DISABLED: Stations section - re-enable by uncommenting. ,"/wiki/stations" */]}/>
  </WikiPage>;
}
