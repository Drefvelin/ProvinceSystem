...
import { Callout, CommandTable, DataTable, RankName, SeeAlso, StatGrid, WikiPage, WikiSectionHeading } from "@/app/components/wiki";
...
import { V } from "../data/helpers";
import type { Recipe } from "../data/types";

const linkClass = "text-[var(--tfmc-accent)] underline";
const ammoName = (name: string) => name.replaceAll("flak_bullet", "Flak Shells").replaceAll("bullet", "Bullet Box").replaceAll("cannonball", "Cannonball").replaceAll("clusterbomb", "Clusterbomb");
...
const engineeringModel = { url:"/wiki/models/vehicles/ammunition_station.json", textures:{
  "0":stationTexture("ammunition_station","bullet"), "1":stationTexture("ammunition_station","pin"), "2":stationTexture("ammunition_station","stand"), "3":stationTexture("ammunition_station","bag"), "4":stationTexture("ammunition_station","wood4"), "5":stationTexture("ammunition_station","powder"), "6":stationTexture("ammunition_station","bullet2"), "8":stationTexture("ammunition_station","wood2"), "9":stationTexture("ammunition_station","wood1"), particle:stationTexture("ammunition_station","bullet"),
} };
...
const constructionStations: Recipe[] = [
  { key:"engineering-table", title:"Engineering Table", station:"Crafting Table", ingredients:[iron,iron,iron,oak,oak,oak,oak,oak,oak], output:{name:"Engineering Table",qty:1,model:engineeringModel} },
  { key:"dockyard", title:"Dockyard", station:"Crafting Table", ingredients:[iron,iron,iron,oak,iron,oak,oak,oak,oak], output:{name:"Dockyard",qty:1,model:dockyardModel} },
];
  ...
  return (
    <WikiPage title="Vehicles & Construction" width="lg" intro="Build a cart, crew a ship or fly an airship. Choose a vehicle below for its materials, build time, fuel and 3D preview.">
      ...
      <ol className="list-decimal space-y-3 pl-5 text-sm text-[var(--tfmc-mist)]">
        ...
        <li>Craft and place an Engineering Table for land vehicles, fixed guns and aircraft, or a Dockyard for ships. Use the crafting-table patterns below.</li>
      ...
      <ol className="list-decimal space-y-3 pl-5 text-sm text-[var(--tfmc-mist)]">
        ...
        <li><strong>Refuel:</strong> hold the fuel listed on the vehicle page and right-click once per item. Each Coal Block or Arcane Fuel adds 100 fuel units. Stop an Arcane Fuel engine before refuelling; Coal Blocks can be added while an engine runs.</li>
      ...
      <p className="text-sm text-[var(--tfmc-mist)]">Hold Vehicle Repair and right-click a grounded, stationary vehicle, or use a mechanic seat to repair while aboard. Use the Repair Tool in the menu to fix components and the Water Bucket for fires. Keep the menu open and stay nearby until repair finishes. Hold Vehicle Paint and right-click to open Select Skin.</p>
      <DataTable className="mt-4" columns={[{ header: "Engineer Station tool" }, { header: "Materials" }, { header: "Time" }, { header: "Requirement" }]}
        rows={[["Vehicle Repair", "4 Iron Ingots", "5 seconds", "Vehicle Mechanic profession"], ["Vehicle Paint", "1 Common Item Skin Scroll", "5 seconds", <>Vehicle Mechanic and <RankName rank="Gilded" /> rank or higher</>]]} />
      ...
      <DataTable className="mt-4" columns={[{ header: "Ammunition" }, { header: "Rounds per item" }]}
        rows={vehicleAmmunition.map((row) => [row[0] === "Bullets" ? "Bullet Box" : row[0], row[3]])} />
      <Callout>Each barrel receives a load of ammunition. One Bullet Box loads 200 rounds into the Biplane's two-barrel Front Rifles. Small Bomb Racks supply eight bombs per load. Mounted weapons use vehicle ammunition from the Engineer Station.</Callout>
      <Callout variant="warning">Cannonballs and bombs can damage terrain. Cannonballs can also start fires and apply Poison for ten seconds.</Callout>
      <WikiSectionHeading id="tracks">Train tracks</WikiSectionHeading>
      <p className="text-sm text-[var(--tfmc-mist)]">Build trains on custom train tracks. Craft Train Track in batches of 64 at the Block Station. Leave room for wide bends and gentle slopes: turns have a 35-degree limit, and slopes have a 10-degree maximum.</p>
      <DataTable className="mt-4" columns={[{ header: "Track item" }, { header: "Use" }]}
        rows={vehicleTrackItems.slice(0, 5).map((row) => [row[0], row[2]])} />
      <WikiSectionHeading id="commands">Commands</WikiSectionHeading>
      <CommandTable commands={vehicleCommands.commands} />
      <SeeAlso hrefs={["/wiki/characters", "/wiki/materials", /* TEMPORARILY DISABLED: Stations section - re-enable by uncommenting. "/wiki/stations", */ "/wiki/commands"]} />
    </WikiPage>
  );
}
