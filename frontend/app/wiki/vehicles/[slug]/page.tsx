...
import { Callout, DataTable, WikiPage, WikiSectionHeading } from "@/app/components/wiki";
  ...
  return (
    <WikiPage title={vehicle.name} intro={vehicle.kind}>
      ...
      <DataTable columns={[{ header: "Requirement" }, { header: "Details" }]} rows={[
        ["Construction station", vehicle.station], ["Category", vehicle.kind],
      ...
      <DataTable className="mt-4" columns={[{ header: "Material" }, { header: "Quantity", align: "right" }]}
        rows={vehicle.inputs.split(", ").map((input) => { const [qty, ...name] = input.split(" "); return [<Link key={input} href="/wiki/materials" className="text-[var(--tfmc-accent)] underline">{name.join(" ")}</Link>, qty]; })} />
      ...
      <DataTable columns={[{ header: "Feature" }, { header: "Details" }]} rows={[
        ...
        ["Fuel", vehicle.fuel], ["Mounted weapons", vehicle.weapons], ["Cargo", vehicle.cargo],
      ]} />
      {vehicle.fuelStats ? <>
        ...
        <Callout>Each fuel item supplies 100 units. Hold {vehicle.fuel} and right-click once per item. {vehicle.fuel === "Arcane Fuel" ? "Stop the engine before adding Arcane Fuel.": "Coal Blocks can be added while the engine runs."} Fuel burn is flat whenever the engine runs at nonzero throttle; using 20% throttle does not save fuel.</Callout>
      ...
      {vehicle.category === "train" ? <Callout variant="warning">Build this vehicle on custom train tracks. See the <Link href="/wiki/vehicles#tracks" className="underline">train-track guide</Link>.</Callout>: null}
