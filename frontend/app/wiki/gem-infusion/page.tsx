import { Callout, ItemGallery, SeeAlso, StatGrid, WikiPage, WikiSectionHeading } from "@/app/components/wiki";
...
export default function GemInfusionPage() {
  return <WikiPage title="Gem Infusion" width="lg" intro="Turn blank gemstones into stat-bearing Infused Gemstones at an Enchanting Table. Goldsmiths can then build jewellery around one infused gem at a Smithing Table.">
    ...
    <ol className="mt-4 list-decimal space-y-3 pl-6 text-sm text-[var(--tfmc-mist)]">
      <li>Bring a Blank Gemstone from the 40-entry catalogue and an Infusion Ticket to any Enchanting Table. An ordinary vanilla jewel will not work.</li>
    ...
    <ol className="mt-4 list-decimal space-y-3 pl-6 text-sm text-[var(--tfmc-mist)]">
      <li>Right-click an empty Smithing Table and choose one of the 14 projects.</li><li>Right-click with each required gold material to deposit one item per click. Follow the exact recipe; all gold belongs to one material bucket, but the finish check still requires the correct mix.</li><li>Right-click with one Infused Gemstone. Every project requires exactly one. Any of the 40 infused gems is accepted.</li><li>Left-click with the Goldsmith Hammer, Small Goldsmith Hammer and Tinker Tool to fill their respective counters. Requirements are calculated from deposited materials.</li><li>Right-click with the Branding Tool to inspect material, gem and hit progress. When every requirement is exactly complete, left-click with it to finish and collect the jewellery dropped on the bench.</li><li>To cancel, shift-left-click with the Branding Tool. Materials and the gem return to inventory; overflow drops at your feet.</li>
    </ol>
    ...
    <SeeAlso hrefs={["/wiki/research","/wiki/magic","/wiki/materials" /* TEMPORARILY DISABLED: Stations section - re-enable by uncommenting. ,"/wiki/stations" */]}/>
  </WikiPage>;
}
