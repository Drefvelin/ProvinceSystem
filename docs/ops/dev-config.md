...
| Province id grid (editor) | `defines/{map}/province_id_grid.bin.gz` | `python -m scripts.tools.build_province_id_grid --map main` from `ProvinceSystem/backend` when `provinces.png` / `provinces.txt` change | Editor requires grid file |
