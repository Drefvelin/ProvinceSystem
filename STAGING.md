...
Temporary ProvinceSystem next to production for Discord bot testing. Uses ports **8001** (API) and **3001** (UI). Production `docker-compose.yml` (8000/3000) is unchanged.
...
```bash
curl -s -X POST http://127.0.0.1:8001/skins/codes \
...
```bash
ssh -L 3001:127.0.0.1:3001 -L 8001:127.0.0.1:8001 user@amp-host
```

Open `http://127.0.0.1:3001/skins`, redeem the code, upload, wait for `#bot-feed` (or `/skinsreview post <id>`).
